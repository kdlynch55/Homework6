...
#if(!file.exists("HW6.R")) {
#  stop("cannot find file 'HW6.R'")
#}
...
test_that('mean function works', {
  result <- 2
  expect_equal({
    x <- as(c(0, 8, 0, 0, 2), "sparse_numeric")
    mean(x)}, result)
})

test_that('mean of zeros works', {
  expect_equal({
...
test_that('norm works', {
  expect_equal({
  x <- as(c(6, 0, 7, 0, 5), "sparse_numeric")
  norm(x)
  }, sqrt(6^2 + 0^2 + 7^2 + 0^2 + 5^2))
})

test_that('norm of zeros works', {
  expect_equal({
    x <- as(c(0,0,0,0,0), "sparse_numeric")
    norm(x)
  }, 0)
...
test_that('standardize works', {
  ...
  }, (c(6,0,7,0,5) - mean(c(6,0,7,0,5)))/sd(c(6,0,7,0,5)))
})
